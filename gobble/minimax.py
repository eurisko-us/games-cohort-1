...
class Minimax:
    def __init__(self,state,pieces, max_player):
        self.root = Node(state, pieces, None,None,0)
            ...
            for y in range(3):
                if base_state[x][y]['size'] < biggest_available_piece:
                    for size in list(set([i for i in base_pieces[player-1] if i > base_state[x][y]['size']])):
                        ...
                        possible_state[x][y]['player'] = {'player':player, 'size': size}
                        possible_pieces[player-1].remove(size)
                        new_node = Node(possible_state,possible_pieces, self.active_node,((x,y),size),self.active_node.depth + 1)
    ...
    def build_tree(self):
        ...
        while len(queue) > 0:
            ...
            if not self.active_node.terminal or self.active_node.depth < 3:
